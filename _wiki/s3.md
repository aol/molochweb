...
Use ```s3AccessKeyId``` and ```s3SecretAccessKey``` settings only if absolutely necessary, i.e. if the machine using the config is not running in EC2, or is running Moloch before 2.1.0. Storing security credentials EC2 instances is not best practice; instead, use IAM Role based authentication.
</div>
